...
const LivePanel = ({ orderItems, logs, isVisible, transcript = [] }: LivePanelProps) => {
  ...
  return (
    <motion.div
      ...
      className="fixed right-4 top-20 bottom-4 w-80 flex flex-col gap-3 z-40"
    >
      ...
      <AnimatePresence>
        {(isVisible || orderItems.length > 0) && (
          <motion.div
            ...
            className="bg-card/90 backdrop-blur-xl rounded-xl border border-border/50 overflow-hidden shadow-xl"
          >
      ...
      <motion.div
        ...
        className="flex-1 bg-card/90 backdrop-blur-xl rounded-xl border border-border/50 overflow-hidden flex flex-col shadow-xl min-h-[200px]"
      >
        <div className="px-4 py-2.5 border-b border-border/30 flex items-center gap-2">
